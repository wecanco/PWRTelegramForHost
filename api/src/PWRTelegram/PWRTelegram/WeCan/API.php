...
class API extends Tools
{
    ...
    public function madeline_connect()
    {
        if (!isset($this->madeline)) {
            ...
            $this->madeline = new \danog\MadelineProto\API(['logger' => ['logger' => 1], 'pwr' => ['pwr' => true, 'db_token' => $this->db_token, 'strict' => true], 'app_info' => ['api_id' => $this->app_id, 'api_hash' => $this->app_hash], 'connection_settings' => ['all' => ['test_mode' => $this->deep]]]);
            //$this->madeline = new \danog\MadelineProto\API(['logger' => ['logger' => 1], 'pwr' => ['pwr' => true, 'db_token' => $this->db_token, 'strict' => true], 'app_info' => ['api_id' => $this->app_id, 'api_hash' => $this->app_hash], 'connection_settings' => ['all' => ['test_mode' => $this->deep,'proxy_extra'=>['ip'=>'94.80.18.140','port'=>12905]]]]);
    ...
    public function madeline_connect_backend()
    {
        if (!isset($this->madeline_backend)) {
            if (!file_exists($this->madeline_backend_path)) {
                $this->jsonexit(['ok' => false, 'error_code' => 400, 'description' => 'Set a custom backend to use the PWRTelegram API. Instructions available @ https://wecan-co.in']);
            }
            require_once $this->pwrhomedir.'/vendor/autoload.php';
            ...
            if ($this->madeline_backend === false) {
                $this->jsonexit(['ok' => false, 'error_code' => 400, 'description' => 'Reset the custom backend to use the PWRTelegram API. Instructions available @ https://wecan-co.in']);
            }
        }
    }
    ...
    public function upload($file, $whattype = 'url', $name = '', $type = '', $forcename = false, $oldparams = [])
    {
        ...
        $path = $this->homedir.'ul/'.$me.'/'.base64_encode($file);
        if (file_exists($file) && $whattype == 'file') {
            ...
            unset($this->pdo);
			if(file_exists($path)){
				return ['ok' => false, 'error_code' => 400, 'description' => "این فایل درحال دانلود است...",'size'=>filesize($path)];
			}
			$wsize = $this->retrieve_remote_file_size($file);
			if($wsize > $this->limitsize){
				return ['ok' => false, 'error_code' => 400, 'description' => "درحال حاظر فایل بیش از 50  مگابایت پردازش نمی شود."];
			}
