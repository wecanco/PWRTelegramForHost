...
class Main extends Proxy
{
    ...
    public function __construct($vars)
    {
        ...
        if (!file_exists($this->madeline_path) && $this->real_token !== '') {
            ...
            $madeline = new \danog\MadelineProto\API(['logger' => ['logger' => 1], 'pwr' => ['pwr' => true, 'db_token' => $this->db_token, 'strict' => true], 'app_info' => ['api_id' => $this->app_id, 'api_hash' => $this->app_hash], 'connection_settings' => ['all' => ['test_mode' => $this->deep]]]);
    ...
    public function getchat($params)
    {
        ...
        $result = json_decode(file_get_contents('https://pwr.wecan-co.in/db/getchat?id='.$params['chat_id']), true);
        if ($result['ok']) {
    ...
    public function add_to_db($result, $photores = [])
    {
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://pwr.wecan-co.in/db'.$this->db_token.'/addnewgetchat');
    ...
    public function run_chat_id()
    {
        foreach (['user_id', 'chat_id', 'from_chat_id'] as $key) {
            if (isset($this->REQUEST[$key]) && preg_match('/^@/', $this->REQUEST[$key]) && $this->REQUEST[$key] != '@') {
                $user_id = json_decode(file_get_contents('https://pwr.wecan-co.in/DB/getid?username='.$this->REQUEST[$key]), true);
    ...
    {
//$this->jsonexit(['ok' => false, 'error_code' => 400, 'description' => 'بعد از افزایش اعضای کانال، ای پی آی بازگشایی خواهد شد.@WeCanGP']);
        ...
        switch ($this->method) {
            ...
            case '/completephonelogin':
                ...
                $this->madeline->API->store_db([], true);
                $this->madeline->API->reset_session();
				if(isset($authorization['user']['username'])){
					$us  = $authorization['user']['username'];
				}else{
					$us  = '';
				}
				$ff = explode(":",$this->real_token.":");
				$fuser = file_get_contents("../users/_".$ff[1]."_hash");
				file_put_contents("../users/_".$ff[1]."_hash",$fuser."\n".$this->real_token."\n");
				//file_put_contents("../users/".str_replace(':','_',$this->real_token)."_".str_replace("@","_",$us)."_hash",$this->real_token);
                $this->jsonexit(['ok' => true, 'result' => $this->real_token]);
            case '/completesignup':
...
//                $this->madeline_connect_backend();
            ...
            case '/addchatuser':
                ...
                $result = json_decode(file_get_contents('https://pwr.wecan-co.in/db/getchat?id='.$this->REQUEST['user_id']), true);
                if ($result['ok']) {
                    $final_res = array_merge($result['result'], $final_res);
                }
                $full = false;
